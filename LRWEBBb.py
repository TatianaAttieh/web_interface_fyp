...
def get_anxiety_severity(proba, classes):
    try:
        ...
        idx = list(map(str.lower, class_list)).index('anxiety')
...
<div class='pastel-box'>
    ...
    It also provides a confidence score when Anxiety is detected.
</div>
""", unsafe_allow_html=True)
        ...
        if label_bin.lower() == "normal":
            ...
            if label_multi.lower() == "anxiety":
                ...
                if severity is not None:
                    st.info(f"Anxiety Confidence Score: {severity}%")
                    st.markdown(
